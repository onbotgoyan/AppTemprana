...
(function() {
    ...
    function iniciarAppsAgente() {
        ...
        style.innerHTML = `
            ...
            .campo-rellenar { color: #004FA8; font-weight: bold; background: #CDFFF2; padding: 0 2px; }
        ...
        // FILA 1: Refresco, Plantillas, SPEECH, REFUT, FRACC + LATENCIA AL FINAL
        const f1 = document.createElement('div');
        f1.className = 'fila-agente';
        const btnRef = document.createElement('button');
        btnRef.className = 'btn-agente'; btnRef.innerText = '🔄'; btnRef.onclick = () => location.reload();
        const configP = [
        ...
        btnB1.className = 'btn-agente'; btnB1.innerText = 'SPEECH';
        const btnB2 = document.createElement('button'); btnB2.className = 'btn-agente'; btnB2.innerText = 'INFO+';
        const btnC = document.createElement('button');
        btnC.className = 'btn-agente'; btnC.style.background = '#3DBB9A'; btnC.innerText = 'FRACC';
        ...
        // LATENCIA AL FINAL DE F1
        const dLat = document.createElement('div');
        dLat.className = 'latencia-box'; dLat.innerText = '...';
        ...
        const iD = document.createElement('input'); iD.className = 'input-calc'; iD.placeholder = '%';
        iD.style.width = "35px";
        const chk = document.createElement('input'); chk.type = 'checkbox';
        const rA = document.createElement('span'); rA.className = 'res-val v-ahorro'; rA.innerText = 'Desc:0';
        const rV = document.createElement('span'); rV.className = 'res-val v-final';
        rV.innerText = 'Monto:0';
        const rM = document.createElement('span'); rM.className = 'res-val v-cuota'; rM.innerText = 'Fracc:0';
        const rS = document.createElement('span');
        rS.className = 'res-val v-total-p'; rS.innerText = 'Plan+Fracc:0';
        const calcLogic = () => {
            const val = parseFloat(iP.value) || 0; const pct = parseFloat(iD.value) || 0;
            let vV, cM, sA, aR;
            if (chk.checked) {
                iD.disabled = false;
                iD.style.background = "white"; iD.style.color = "black"; iP.placeholder = "Exigible";
                aR = val * (pct / 100);
                vV = val - aR; cM = vV / 6; sA = val + cM; rA.innerText = `Desc:${aR.toFixed(1)}`;
            } else {
                iD.disabled = true;
                iD.style.background = "#7f8c8d"; iD.style.color = "white"; iP.placeholder = "Yape";
                aR = 0; vV = val; cM = vV / 6;
                sA = (vV / 0.75) + cM; rA.innerText = `Desc:-`;
            }
            rV.innerText = `Monto:${vV.toFixed(1)}`; rM.innerText = `Fracc:${cM.toFixed(1)}`;
            rS.innerText = `Plan+Fracc:${sA.toFixed(1)}`;
        };
        ...
        f3.className = 'fila-agente';
        const selAg = document.createElement('select'); selAg.className = 'select-agente-reg'; selAg.style.width = "80px";
        const optDefA = document.createElement('option');
        optDefA.innerText = "👤 AGENTE"; selAg.append(optDefA);
        const BLOQUEO_PERMANENTE = true;

        const inM = document.createElement('input'); inM.className = 'input-reg'; inM.placeholder = 'Móvil';
        inM.style.width = "65px";
        const inF = document.createElement('input'); inF.type = 'date'; inF.className = 'input-reg'; inF.style.width = "65px";
        const btnR = document.createElement('button');
        btnR.className = 'btn-agente'; btnR.style.background = '#3DBB9A'; btnR.innerText = 'PDP';
        ...
        const dAvanceF = document.createElement('div'); dAvanceF.className = 'avance-box';
        dAvanceF.innerText = '...'; dAvanceF.style.color = '#0b518f';
        ...
        // --- INTERVENCIÓN QUIRÚRGICA: Se eliminaron los intervalos automáticos ---
        // setInterval(actualizarAvance, 2000);
        // setInterval(actualizarAvancePlus, 2000);
        // setInterval(actualizarAvanceF, 2000);
        ...
        btnR.onclick = async () => {
            if(!selAg.value || !inM.value || !inF.value) { btnR.style.background = '#e67e22';
                setTimeout(()=>btnR.style.background='#3DBB9A', 1000); return; }
            ...
            } catch (e) { btnR.style.background = '#c0392b';
                setTimeout(() => btnR.style.background = '#3DBB9A', 2000); } finally {
                btnR.innerText = "PDP";
                // --- INTERVENCIÓN QUIRÚRGICA: Actualización 10 segundos después de ejecutar PDP ---
                setTimeout(() => {
                    actualizarAvance();
        ...
        selEst.className = 'select-agente-reg'; selEst.style.width = "80px";
        ...
        const copiarEstrategia = () => {
            ...
            if (textoOriginal !== "---" && textoOriginal !== "..." && textoOriginal !== "¡Copiado!") {
                ...
                resCola.innerText = "¡Copiado!";
                resCola.style.color = "#3DBB9A";
        ...
        const consultarEstrategia = async () => {
            ...
            try {
                ...
                let valE = raw, valQ = "-";
                if (raw.includes('|')) { const partes = raw.split('|');
                valE = partes[0].trim(); valQ = partes[1].trim(); }
                if (peticionActual !== parametro) return;
                const savedTime = localStorage.getItem(`last_copy_${selEst.value}`);
                if (ultimoValorE !== "" && ultimoValorE !== valE) { iconMsg.innerText = "📩"; iconMsg.style.fontSize = "14px";
                iconMsg.style.display = 'inline'; }
                else if (savedTime) { iconMsg.innerText = savedTime;
                iconMsg.style.fontSize = "10px"; iconMsg.style.display = 'inline'; }
                if (resCola.innerText !== "¡Copiado!") { ultimoValorE = valE;
                resCola.innerText = valE; resCuartil.innerText = `C: ${valQ}`; }
